    ...
    public function checkDailyLogin(){
        ...
        foreach($subscriptions as $key => $subscription){
            ...
            foreach($dateRange->toArray() as $pastDate){
                ...
                $logged_in  = ActivityLogs::where('log_name', 'login')->where('causer_id', $this->id)->whereBetween('created_at', [$pastDate->startOfDay()->format('Y-m-d H:s:i'), $pastDate->endOfDay()->format('Y-m-d H:s:i')])->first();
